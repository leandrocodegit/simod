...
export class AppConfigurator {
  ...
  ngOnInit() {
    ...
    if (userJson) {
      ...
      if (user?.color)
        this.layoutService.layoutConfig.update((state) => ({ ...state, primary: user.color }));
      if (user?.dark)
        this.layoutService.layoutConfig.update((state) => ({ ...state, darkTheme: !state.darkTheme }));
      this.applyTheme('primary', user.color);
    }
  ...
  primaryColors = computed<SurfacesType[]>(() => {
    ...
    const colors = ['gray', 'emerald', 'green', 'lime', 'orange', 'amber', 'yellow', 'teal', 'cyan', 'sky', 'blue', 'indigo', 'violet', 'purple', 'fuchsia', 'pink', 'rose', 'red'];
